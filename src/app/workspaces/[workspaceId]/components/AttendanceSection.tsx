...
export default function AttendanceSection({ streak: initialStreak, startDate, endDate, attendances, minStudyHours = 0, onCheckInComplete }: AttendanceSectionProps) {
    ...
    const fetchTableConfig = async () => {
        try {
            const res = await fetch(`/api/workspaces/${workspaceId}/config`, { cache: 'no-store' });
            if (res.ok) {
                const data = await res.json();
                const { tableConfig, extraRows } = data;
                ...
                // Restore Extra Rows
                // Logic for extraRows is handled by normalizedExtraRows in fetchData, 
                // but we return it here to be used there.

                return { tableConfig, extraRows }; // Data now includes potential 'examples' in tableConfig if saved
            }
        } catch (e) {
    ...
    const saveTableConfig = async (currentCols: string[], currentRows: string[][]) => {
        // Find max date logic...
        let maxDate = '';
        ...
        try {
            await fetch(`/api/workspaces/${workspaceId}/config`, {
                ...
                body: JSON.stringify({ tableConfig, extraRows })
            });
        } catch (e) {
    ...
    const fetchData = async () => {
        try {
            // Load Config First
            const configData = await fetchTableConfig();
            let currentCols = INITIAL_COLUMNS;
            let extraRows: string[][] = [];
            let savedMaxDate: Date | null = null;
            let savedExamples: Record<string, string> = {};

            if (configData) {
                const { tableConfig, extraRows: savedExtraRows } = configData;
                if (tableConfig && tableConfig.columns) {
                    setColumns(tableConfig.columns);
                    currentCols = tableConfig.columns;
                }
                if (savedExtraRows) {
                    extraRows = savedExtraRows;
                }
                if (tableConfig && tableConfig.maxDate) {
                    savedMaxDate = new Date(tableConfig.maxDate);
                }
                if (tableConfig && tableConfig.examples) {
                    savedExamples = tableConfig.examples;
                }
            }

            const res = await fetch(`/api/workspaces/${workspaceId}/attendance`, { cache: 'no-store' });
            if (res.ok) {
                const data = await res.json();
                ...
                setRows(prevRows => {
                    ...
                    const buildRow = (dateKey: string, dateLabel: string) => {
                        ...
                        if (serverAtt) {
                            const isDraft = serverAtt.note === 'DRAFT' || (serverAtt.note && serverAtt.note.startsWith('DRAFT:'));
                            ...
                            if (!isDraft) {
                                ...
                                // It is a draft
                                let cleanNote = serverAtt.note === 'DRAFT' ? '' : serverAtt.note;
                                if (cleanNote && cleanNote.startsWith('DRAFT:')) {
                                    cleanNote = cleanNote.replace('DRAFT:', '');
                                }

                                if (localAtt) {
                                    finalStart = localAtt.start || serverAtt.start;
                                    finalEnd = localAtt.end || serverAtt.end;
                                    finalNote = localAtt.note || cleanNote || '';
                                    finalStatus = '작성 중... 💾';
                                } else {
                                    finalStart = serverAtt.start;
                                    finalEnd = serverAtt.end;
    ...
    const saveDraft = async (rowIndex: number) => {
        ...
        if (!hasData) {
            ...
            // If user cleared text, ensure we delete draft
            try {
        ...
        // 2. Local Save (Save BEFORE validation so invalid drafts are kept)
        saveLocalDraft(dateStr, { start: startTime, end: endTime, note, extras });

        // Validate Time Format (HH:mm)
        const timeRegex = /^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/;
        if (startTime && !timeRegex.test(startTime)) {
            // Invalid Start Time - Keep local edit but skip Server API
            return;
        }
        if (endTime && !timeRegex.test(endTime)) {
            // Invalid End Time - Keep local edit but skip Server API
            return;
        }
    ...
    const deleteRow = async (rowIndex: number) => {
        ...
        try {
            const dateObj = new Date(dateStr);
            // Since we can't be sure if it's a valid date object just from string manipulation if it's custom text
            // But our system enforces YYYY-MM-DD for date rows.
            if (/^\d{4}-\d{2}-\d{2}/.test(dateStr)) {
                await fetch(`/api/workspaces/${workspaceId}/attendance/check-in`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
        ...
        } catch (e) { console.error("Failed to delete row data", e); }
    ...
    return (
        <>
            ...
            <section className={styles.section} style={{ width: '100%', maxWidth: '1200px', margin: '0 auto', overflowX: 'auto', padding: '0', position: 'relative' }}>
                {renderMenu()}
                ...
                >
                    {isLoaded ? (
                        <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, fontSize: '0.9rem', textAlign: 'center', minWidth: `${550 + 300 + (Math.max(0, columns.length - 5) * 150)}px`, tableLayout: 'fixed' }}>
                            <thead>
                                <tr style={{ backgroundColor: '#27272a' }}>
                                    {columns.map((col, colIndex) => {
                                        ...
                                        // Width Logic
                                        let widthStyle = {};
                                        if (colIndex === 0) widthStyle = { width: '150px' }; // Date
                                        else if (colIndex === 1 || colIndex === 2) widthStyle = { width: '110px' }; // Time
                                        else if (colIndex === 3) widthStyle = { width: 'auto' }; // Reason (Flexible)
                                        else if (colIndex === 4) widthStyle = { width: '130px' }; // Status
                                        else widthStyle = { width: '150px' }; // Custom Columns
                                        ...
                                        return (
                                            <th
                                                ...
                                                style={{
                                                    ...
                                                    // minWidth: '120px', // Removed general minWidth in favor of specific widths
                                                    ...widthStyle,
