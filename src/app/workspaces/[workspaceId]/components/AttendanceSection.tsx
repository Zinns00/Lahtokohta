...
export default function AttendanceSection({ streak: initialStreak, startDate, endDate, attendances, minStudyHours = 0, onCheckInComplete }: AttendanceSectionProps) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (extraRows && Array.isArray(extraRows)) {
                    setRows(prev => {
                        // Merge extra rows into existing rows (which are date-generated)
                        // This logic might need refinement depending on how rows are generated
                        // For now, we assume rows are regenerated by fetchData and we append extraRows?
                        // Actually, fetchData sets rows. We should likely integrate this into fetchData or run after it.
                        // Let's store extraRows in a ref or state to appended when rows are rebuilt.
                        return prev;
                    });
                    // Note: Ideally, we pass extraRows to fetchData or handle merging there. 
                    // But to keep it simple, let's just save them in a state to be used during row generation if needed, 
                    // or just appened them if they are static.
                    // Accessing fetchData's logic would be better.
                    // For this refactor, let's look at how loadTableConfig was used.
                    // It was used in useEffect. 
                }
                return { tableConfig, extraRows };
            }
        } catch (e) {
    ...
    const saveDraft = async (rowIndex: number) => {
        ...
        if (startTime && !timeRegex.test(startTime)) {
            // Invalid Start Time - Skip API but keep local edit for user correction
            return;
        }
        if (endTime && !timeRegex.test(endTime)) {
            // Invalid End Time - Skip API but keep local edit
            return;
        }
